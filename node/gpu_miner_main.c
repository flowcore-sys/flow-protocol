...
static int g_intensity = 100;       /* 1-100, controls GPU batch size */
...
int main(int argc, char* argv[])
{
    ...
    uint32_t batch_size = (FTC_GPU_BATCH_SIZE * g_intensity) / 100;
    g_farm = ftc_gpu_farm_new(g_device_mask, batch_size);
    if (g_intensity < 100) {
        printf("Mining intensity: %d%% (for laptop cooling)\n", g_intensity);
    }
    if (!g_farm) {
