...
set BUILD_DIR=c:\Flow Protocol\build

rd /s /q "%BUILD_DIR%" 2>nul
mkdir "%BUILD_DIR%"

echo Compiling CUDA kernel...
"%CUDA_PATH%\bin\nvcc.exe" -c "%SRC_DIR%\src\miner\keccak256_cuda.cu" -o "%BUILD_DIR%\keccak256_cuda.obj" ^
    -I"%SRC_DIR%\include" -I"%SRC_DIR%\src" -I"%CUDA_PATH%\include" ^
...
echo.
echo SUCCESS! Binary: %BUILD_DIR%\ftc-miner-gpu.exe
copy ftc-miner-gpu.exe "%SRC_DIR%\release\"
goto end
